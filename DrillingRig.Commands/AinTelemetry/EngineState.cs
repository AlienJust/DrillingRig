...
namespace DrillingRig.Commands.AinTelemetry {
	public enum EngineState
	{
		...
		TestImcw,             // 3 Проверка ролей АИН.
		PreResetErrors,        //4 Подготовка к Сбросу аварий АИН3,2,1.

		ResetErrors,						// 4 Сброс аварий АИН2,3,1 (ведущий - последний!).
		NotReady,							// 5 НЕ Готов к запуску двигателя.
		ReadyToSwitchOn,			// 6 Готов к работе.
		ReadyRun,									// 7 Готов к запуску двигателя.
		
		EnableOperation, // 8 Запущен ШИМ, RAMP_OUT_ZERO.
		EnableOutput, // 9 Запущен ШИМ, RAMP_HOLD
		AcceleratorEnable, // 10 Запущен ШИМ, RAMP_IN_ZERO
		OperatingState, // 11 Запущен двигатель с ненулевой скоростью

		ReadySlave, // 12 Готов к запуску двигателя с управлением от мастера по CAN
